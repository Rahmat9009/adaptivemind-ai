...
export type StudyPlanGoal = "review" | "continue" | "exam" | "habit" | "explore";
export type StudyIntensity = "light" | "balanced" | "focused";
export type StudyTaskType = "review" | "lesson" | "practice" | "understanding-check" | "reflection";
export interface StudyTask { id: string; topic: string; type: StudyTaskType; minutes: number; reason: string; teachingApproach: LearningDimension[]; completed: boolean; }
export interface StudyPlanDay { dayNumber: number; date?: string; totalMinutes: number; focus: string; tasks: StudyTask[]; }
export interface StudyPlan { id: string; createdAt: string; goal: StudyPlanGoal; durationDays: number; minutesPerDay: number; intensity: StudyIntensity; days: StudyPlanDay[]; summary: string; }
export interface StudyPlanSettings { goal: StudyPlanGoal; availableDays: number[]; minutesPerDay: number; durationDays: 3 | 7 | 14; intensity: StudyIntensity; targetDate?: string; priorities?: string; }
...
export function readStudyPlan(): StudyPlan | null { try { const value: unknown = JSON.parse(localStorage.getItem(studyPlanStorageKey) ?? "null"); return typeof value === "object" && value !== null && validateStudyPlan(value as StudyPlan) ? value as StudyPlan : null; } catch { return null; } }
export function saveStudyPlan(plan: StudyPlan) { localStorage.setItem(studyPlanStorageKey, JSON.stringify(plan)); }
